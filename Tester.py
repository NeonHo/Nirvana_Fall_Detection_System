...
class Tester:
    def __init__(self):
        ...
        self.optical_generator = OpticalGenerator(self.video_path, self.flow_image_path, self.bound, self.width,
                                                  self.height, self.feature_extractor_1.stack_length)
    ...
    def exam(self) -> None:
        ...
        feature_queue = Queue()
        avi_path = "F:\\fsociety\\graduation_project\\Nirvana_Fall_Detection_System\\test_ground\\video_nofall.avi"
        # Thread(target=self.videographer.capture_video, args=(frame_queue,)).start()
        Thread(target=self.optical_generator.generate_optical_flow_farneback, args=(avi_path, flow_queue_0, flow_queue_1,)).start()
