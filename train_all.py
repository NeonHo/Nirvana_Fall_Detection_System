...
def main():
    ...
    # Undersample the FDD and Multicam: take 0s and 1s per dataset and undersample each of them separately by random sampling without replacement
    ...
    all1_multicam = np.random.choice(all1_multicam, size_1, replace=False)
    all0_urfd = np.random.choice(all0_urfd, size_0, replace=False)
    all1_urfd = np.random.choice(all1_urfd, size_1, replace=False)
    all0_fdd = np.random.choice(all0_fdd, size_0, replace=False)
    all1_fdd = np.random.choice(all1_fdd, size_1, replace=False)
    ...
    for fold in range(5):
        ...
        if use_validation:
            # Multicam
            (train0_multicam, train1_multicam, val0_multicam, val1_multicam) = divide_train_val(train0_multicam, train1_multicam, val_size // 3)
            temp = np.concatenate((val0_multicam, val1_multicam))
            X_val_multicam = X_multicam[temp]
            y_val_multicam = y_multicam[temp]
            # URFD
            (train0_urfd, train1_urfd, val0_urfd, val1_urfd) = divide_train_val(train0_urfd, train1_urfd, val_size // 3)
        ...
        # Sampling
        X_train_multicam, y_train_multicam = sample_from_dataset(X_multicam, y_multicam, train0_multicam, train1_multicam)
        X_train_urfd, y_train_urfd = sample_from_dataset(X_urfd, y_urfd, train0_urfd, train1_urfd)
        ...
        X_test_multicam, y_test_multicam = sample_from_dataset(X_multicam, y_multicam, test0_multicam, test1_multicam)
        X_test_urfd, y_test_urfd = sample_from_dataset(X_urfd, y_urfd, test0_urfd, test1_urfd)
    ...
    print("Sensitivity Combined: {:.2f}% (+/- {:.2f}%)".format(np.mean(sensitivities['combined']) * 100., np.std(sensitivities['combined']) * 100.))
    print("Specificity Combined: {:.2f}% (+/- {:.2f}%)\n".format(np.mean(specificities['combined']) * 100., np.std(specificities['combined']) * 100.))
    print("Sensitivity URFD: {:.2f}% (+/- {:.2f}%)".format(np.mean(sensitivities['urfd']) * 100., np.std(sensitivities['urfd']) * 100.))
    print("Specificity URFD: {:.2f}% (+/- {:.2f}%)\n".format(np.mean(specificities['urfd']) * 100., np.std(specificities['urfd']) * 100.))
    print("Sensitivity Multicam: {:.2f}% (+/- {:.2f}%)".format(np.mean(sensitivities['multicam']) * 100., np.std(sensitivities['multicam']) * 100.))
    print("Specificity Multicam: {:.2f}% (+/- {:.2f}%)\n".format(np.mean(specificities['multicam']) * 100., np.std(specificities['multicam']) * 100.))
    print("Sensitivity Multicam: {:.2f}% (+/- {:.2f}%)".format(np.mean(sensitivities['fdd']) * 100., np.std(sensitivities['fdd']) * 100.))
    print("Specificity FDDs: {:.2f}% (+/- {:.2f}%)".format(np.mean(specificities['fdd']) * 100., np.std(specificities['fdd']) * 100.))
